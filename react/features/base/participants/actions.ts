...
export function localParticipantRoleChanged(role: string) {
    return (dispatch: IStore["dispatch"], getState: IStore["getState"]) => {
        ...
        const getExistingRoom = localStorage.getItem(
            `${conference?.room?.toLowerCase()}`
        );
        const jitsiMeetId = localStorage.getItem("jitsiMeetId");

        if (participant) {
            dispatch(participantRoleChanged(participant.id, role));

            if (!getExistingRoom && role == PARTICIPANT_ROLE.MODERATOR) {
                const localRole = JSON.stringify({
                    role,
                    jitsiMeetId,
                });
