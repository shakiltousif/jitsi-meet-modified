...
const DeepLinkingMobilePage: React.FC<WithTranslation> = ({ t }) => {
    const deeplinkingCfg = useSelector(
        (state: IReduxState) =>
            ...
            ({} as IDeeplinkingConfig)
    );
    const { hideLogo } = deeplinkingCfg;
    const deepLinkingUrl: string = useSelector(generateDeepLinkingURL);
    const room = useSelector((state: IReduxState) =>
        decodeURIComponent(state["features/base/conference"].room || "")
    );
    const url = useSelector(
        (state: IReduxState) => state["features/base/connection"] || {}
    );
    ...
    const onOpenLinkProperties = useMemo(() => {
        const { downloadLink } = (deeplinkingCfg?.[
            Platform.OS as keyof typeof deeplinkingCfg
        ] || {}) as IDeeplinkingMobileConfig;
